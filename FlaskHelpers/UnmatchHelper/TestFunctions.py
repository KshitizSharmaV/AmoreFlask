...
from FlaskHelpers.UnmatchHelper.UnmatchHelper import unmatch_test_function
...
def match_two_profiles(user_id_1, user_id_2):
    ...
    db.collection("LikesDislikes").document(user_id_1).collection("Match").document(user_id_2).set(
        {"id": user_id_2, "timestamp": time.time()})
    db.collection("LikesDislikes").document(user_id_2).collection("Match").document(user_id_1).set(
        {"id": user_id_1, "timestamp": time.time()})
    ...
    db.collection("RecentChats").document(user_id_1).collection("Messages").document(user_id_2).set(
        {"fromId": user_id_1, "toId": user_id_2,
         "timestamp": datetime.now(), "lastText": "",
         "user": {"firstName": giver_profile["firstName"], "lastName": giver_profile["lastName"],
                  "image1": giver_profile["image1"], "id": user_id_2}, "otherUserUpdated": True})

    db.collection("RecentChats").document(user_id_2).collection("Messages").document(user_id_1).set(
        {"fromId": user_id_2, "toId": user_id_1,
         "timestamp": datetime.now(), "lastText": "",
         "user": {"firstName": receiver_profile["firstName"], "lastName": receiver_profile["lastName"],
